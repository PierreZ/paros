...
//!
//! Usage: `sim-paros-hunt [network|main|snapshot|bounds] [iterations]`
...
use paros_sim::{
    chain_smoke, explore_snapshot_recovery, network_hunt, protocol_bounds_hunt, run_chain_seed,
...
fn main() {
    ...
    if let "replay-network" | "replay-main" | "replay-snapshot" | "replay-bounds" = axis.as_str() {
    ...
    let report = match axis.as_str() {
        ...
        other => {
            eprintln!(
                "unknown axis: {other} (expected 'network', 'main', 'snapshot', or 'bounds')"
            );
            std::process::exit(2);
        }
    };
